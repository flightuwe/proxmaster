...
func (e *Engine) Classify(tool string, params map[string]any) models.RiskLevel {
	...
	case "node.set_maintenance", "vm.migrate", "vm.migration.plan", "updates.rollout_pause", "updates.plan", "policy.simulate", "policy.explain", "vm.create", "vm.clone_from_template", "lxc.create", "storage.inventory.sync", "storage.health.explain", "backup.policy.explain", "backup.policy.list", "backup.target.list", "gitops.sync.now", "gitops.rollback", "ssh.breakglass.disable", "vpn.wireguard.plan", "workload.spec.apply", "service.wireguard.vm.deploy":
